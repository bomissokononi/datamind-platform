            ...
            if len(num_cols) < 2:
                ...
                if features_reg and st.button("🚀 Entraîner les modèles de régression", key="reg_run"):
                    ...
                    for i, (name, model) in enumerate(models_reg.items()):
                        ...
                        # Cross-validation régression sécurisée
                        try:
            ...
            if not cat_cols:
                ...
                if features_clf and st.button("🚀 Entraîner les modèles de classification", key="clf_run"):
                    ...
                    for i,(name,mdl) in enumerate(models_clf.items()):
                        ...
                        # Cross-validation sécurisée — ne plante jamais
                        try:
                            unique_all, counts_all = np.unique(y_clf, return_counts=True)
                            safe_folds = int(min(5, counts_all.min()))
                            if safe_folds >= 2 and len(y_clf) >= safe_folds * 2:
                                cv_acc = cross_val_score(mdl, X_clf_s, y_clf, cv=safe_folds, scoring="accuracy").mean()
                            else:
                                cv_acc = acc  # pas assez de données → on utilise accuracy du test
                        except Exception:
                            cv_acc = acc
